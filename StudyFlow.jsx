...
// ── ENV CONFIG ─────────────────────────────────────────────────────
// These are injected by Vercel/Netlify at build time.
// Create a .env file (never commit it!) with:
//   VITE_GEMINI_API_KEY=your_key_here
//   VITE_ADMIN_EMAIL=your@email.com
//
// For plain HTML + Netlify, use _redirects + netlify.toml env vars,
// then swap "import.meta.env.VITE_*" for "window.__env.*" if needed.
const GEMINI_API_KEY = (typeof import !== "undefined" && import.meta?.env?.VITE_GEMINI_API_KEY) || window.__env?.GEMINI_API_KEY || "";
const ADMIN_EMAIL    = (typeof import !== "undefined" && import.meta?.env?.VITE_ADMIN_EMAIL)    || window.__env?.ADMIN_EMAIL    || "";

// ── FIREBASE ───────────────────────────────────────────────────────
// Same pattern — injected at build time, never hardcoded here.
...
// ── FIRESTORE HELPERS ──────────────────────────────────────────────
const fsSubjectsRef = () => db.collection("subjects").doc("shared");
const fsScoresRef   = (uid) => db.collection("scores").doc(uid);

async function loadSubjectsFromFS() {
  try {
    const snap = await fsSubjectsRef().get();
    return snap.exists ? (snap.data().list || []) : [];
  } catch { return []; }
}

async function saveSubjectsToFS(subjects) {
  try { await fsSubjectsRef().set({ list: subjects }); } catch (e) { console.error("Firestore save error:", e); }
}

async function loadScoresFromFS(uid) {
  try {
    const snap = await fsScoresRef(uid).get();
    return snap.exists ? (snap.data().scores || []) : [];
  } catch { return []; }
}

async function saveScoresToFS(uid, scores) {
  try { await fsScoresRef(uid).set({ scores }); } catch (e) { console.error("Score save error:", e); }
}

// ── ADMIN ROLE via Firebase Custom Claims ──────────────────────────
// IMPORTANT: You must set custom claims on the server side.
// In your Firebase Admin SDK (Cloud Function or script), run once:
//
//   admin.auth().setCustomUserClaims(uid, { role: "admin" })
//
// This function reads that claim from the token — it CANNOT be
// faked by the client, unlike an email string comparison.
async function getUserRole(firebaseUser) {
  const token = await firebaseUser.getIdTokenResult(true); // force refresh
  return token.claims.role === "admin" ? "admin" : "student";
}
...
// Old logic: any string > 5 chars = correct. Meaningless.
// New logic: basic keyword overlap between answer and model answer.
function scoreShortAnswer(userAnswer, modelAnswer) {
  ...
  const matches = modelWords.filter(w => userWords.has(w)).length;
  const overlap = matches / modelWords.length;
  // ≥40% keyword overlap → full credit, ≥15% → half credit, else 0
  return overlap >= 0.4 ? 1 : overlap >= 0.15 ? 0.5 : 0;
}
...
function injectCSS(dark) {
  ...
  el.textContent = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .sf-score-chip{padding:4px 10px;border-radius:8px;font-size:.75rem;font-weight:600}
  `;
}
...
function Navbar({ user, dark, onToggle, onLogout }) {
  const acc = dark ? "#4FFFB0" : "#6366F1";
  return (
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,height:64,padding:"0 28px",
      ...
      <div style={{display:"flex",alignItems:"center",gap:11}}>
        <div style={{width:34,height:34,borderRadius:10,background:dark?"rgba(79,255,176,0.13)":"rgba(99,102,241,0.12)",
      ...
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <button onClick={onToggle} style={{width:40,height:40,borderRadius:12,fontSize:17,
...
function Login({ dark }) {
  ...
  const handleAuth = async (action) => {
    ...
    try {
      ...
      setErr(error.message.replace("Firebase: ","").replace(/\(auth\/.*?\)/,"").trim());
    }
    setLoad(false);
  };
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24,position:"relative",zIndex:1}}>
      <div className="sf-glass sf-up" style={{width:"100%",maxWidth:420,padding:"52px 44px"}}>
        ...
        <div style={{display:"flex",gap:4,marginBottom:24,background:dark?"rgba(255,255,255,0.04)":"rgba(99,102,241,0.07)",borderRadius:50,padding:4}}>
          {[["login","Log In"],["signup","Sign Up"]].map(([k,v])=>(
            <button key={k} onClick={()=>setMode(k)} style={{flex:1,padding:"9px 0",borderRadius:46,border:"none",cursor:"pointer",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".84rem",transition:"all .2s",
              background:mode===k?acc:"transparent",color:mode===k?(dark?"#06090F":"#fff"):"inherit",opacity:mode===k?1:.5}}>{v}</button>
          ))}
        </div>

        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div>
            <label className="sf-syne" style={{display:"block",fontSize:".75rem",fontWeight:700,opacity:.5,letterSpacing:".07em",textTransform:"uppercase",marginBottom:7}}>Email</label>
            <input className="sf-inp" type="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)} disabled={load}/>
          </div>
          <div>
            ...
            <input className="sf-inp" type="password" placeholder="Min 6 characters" value={pw} onChange={e=>setPw(e.target.value)} disabled={load}
              onKeyDown={e=>e.key==="Enter"&&!load&&email&&pw&&handleAuth(mode)}/>
          </div>

          {err && <p style={{color:"#FF5C7A",fontSize:".86rem",padding:"10px 14px",borderRadius:10,background:"rgba(255,92,122,0.08)"}}>{err}</p>}

          <button className="sf-btn sf-btn-p" onClick={()=>handleAuth(mode)} disabled={load||!email||!pw}
            style={{justifyContent:"center",marginTop:4,padding:"14px 0"}}>
            {load ? <><div className="sf-spin"/>Processing…</> : mode==="login" ? "Log In →" : "Create Account →"}
          </button>

          <div style={{position:"relative",margin:"8px 0",textAlign:"center"}}>
            <hr style={{borderColor:dark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.1)"}}/>
            <span className="sf-syne" style={{position:"absolute",top:-9,left:"50%",transform:"translateX(-50%)",
              background:dark?"#06090F":"#EEF0FF",padding:"0 10px",fontSize:".75rem",fontWeight:700,opacity:.5,letterSpacing:".05em"}}>OR</span>
          </div>

          <button className="sf-btn sf-btn-g" onClick={()=>handleAuth("google")} disabled={load}
            style={{width:"100%",justifyContent:"center",padding:"14px 28px"}}>
            {load ? <div className="sf-spin"/> : <img src="https://www.svgrepo.com/show/475656/google-color.svg" alt="Google" style={{width:18}}/>}
            Sign in with Google
          </button>
        </div>
      </div>
...
function Admin({ subjects, setSubjects, dark, onQuiz }) {
  ...
  // Persist to Firestore whenever subjects change
  const updateSubjects = useCallback(async (updater) => {
    setSaving(true);
    setSubjects(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      saveSubjectsToFS(next).finally(()=>setSaving(false));
      return next;
    });
  }, [setSubjects]);
  ...
  const addSubject = () => {
    ...
    updateSubjects(p=>[...p,{id:Date.now(),name:newName,icon:newIcon,color:palette[p.length%palette.length],units:[]}]);
    setNewName(""); setNewIcon("📖"); setAddMode(null);
  };
  const editSubject = (sub) => { const n=prompt("Edit Subject Name:",sub.name); if(n) updateSubjects(p=>p.map(s=>s.id===sub.id?{...s,name:n}:s)); };
  const deleteSubject = (id) => { if(confirm("Delete this subject and all its units?")){ updateSubjects(p=>p.filter(s=>s.id!==id)); if(selSub?.id===id){setSelSub(null);setSelUnit(null);setSelTopic(null);} }};

  const addUnit = () => {
    if(!newName.trim()||!selSub) return;
    updateSubjects(p=>p.map(s=>s.id===selSub.id?{...s,units:[...s.units,{id:Date.now(),name:newName,topics:[]}]}:s));
    setNewName(""); setAddMode(null);
  };
  const editUnit = (subId,unit) => { const n=prompt("Edit Unit Name:",unit.name); if(n) updateSubjects(p=>p.map(s=>s.id===subId?{...s,units:s.units.map(u=>u.id===unit.id?{...u,name:n}:u)}:s)); };
  const deleteUnit = (subId,unitId) => { if(confirm("Delete this unit?")){ updateSubjects(p=>p.map(s=>s.id===subId?{...s,units:s.units.filter(u=>u.id!==unitId)}:s)); if(selUnit?.id===unitId){setSelUnit(null);setSelTopic(null);} }};

  const addTopic = (sub,unit) => {
    if(!newName.trim()) return;
    updateSubjects(p=>p.map(s=>s.id===sub.id?{...s,units:s.units.map(u=>u.id===unit.id?{...u,topics:[...u.topics,{id:Date.now(),name:newName,material:"",questions:[]}]}:u)}:s));
    setNewName(""); setAddMode(null);
  };
  const editTopic = (subId,unitId,topic) => { const n=prompt("Edit Topic Name:",topic.name); if(n) updateSubjects(p=>p.map(s=>s.id===subId?{...s,units:s.units.map(u=>u.id===unitId?{...u,topics:u.topics.map(t=>t.id===topic.id?{...t,name:n}:t)}:u)}:s)); };
  const deleteTopic = (subId,unitId,topicId) => { if(confirm("Delete this topic?")){ updateSubjects(p=>p.map(s=>s.id===subId?{...s,units:s.units.map(u=>u.id===unitId?{...u,topics:u.topics.filter(t=>t.id!==topicId)}:u)}:s)); if(selTopic?.id===topicId) setSelTopic(null); }};

  const saveMat = () => {
    if(!selTopic) return;
    updateSubjects(p=>p.map(s=>s.id===selSub.id?{...s,units:s.units.map(u=>u.id===selUnit.id?{...u,topics:u.topics.map(t=>t.id===selTopic.id?{...t,material:matText}:t)}:u)}:s));
  ...
  const doGenerate = async () => {
    ...
    try {
      ...
      updateSubjects(p=>p.map(s=>s.id===selSub.id?{...s,units:s.units.map(u=>u.id===selUnit.id?{...u,topics:u.topics.map(t=>t.id===selTopic.id?{...t,material:matText,questions:[...t.questions,...qs]}:t)}:u)}:s));
      setGenMsg(`✓ ${qs.length} questions generated!`);
    } catch(e){ setGenMsg(`✗ ${e.message}`); }
    setGen(false); setTimeout(()=>setGenMsg(""),5000);
  };

  const clearQs = () => { if(!selTopic) return; updateSubjects(p=>p.map(s=>s.id===selSub.id?{...s,units:s.units.map(u=>u.id===selUnit.id?{...u,topics:u.topics.map(t=>t.id===selTopic.id?{...t,questions:[]}:t)}:u)}:s)); };
  ...
  return (
    <div style={{paddingTop:64,minHeight:"100vh",position:"relative",zIndex:1}}>
      {/* Saving indicator */}
      {saving && (
        <div style={{position:"fixed",bottom:20,right:20,zIndex:300,padding:"10px 16px",borderRadius:12,
          background:dark?"rgba(6,9,15,0.9)":"rgba(238,240,255,0.9)",border:`1px solid ${dark?"rgba(255,255,255,0.1)":"rgba(99,102,241,0.2)"}`,
          display:"flex",alignItems:"center",gap:8,fontSize:".84rem"}}>
          <div className="sf-spin" style={{borderTopColor:acc}}/>Syncing to cloud…
        </div>
      )}

      {/* Tab bar */}
      <div style={{borderBottom:`1px solid ${dark?"rgba(255,255,255,0.07)":"rgba(99,102,241,0.12)"}`,padding:"0 28px",
        display:"flex",gap:2,background:dark?"rgba(6,9,15,0.6)":"rgba(238,240,255,0.6)",backdropFilter:"blur(10px)"}}>
      ...
      <div style={{padding:"32px 32px 48px",maxWidth:1100,margin:"0 auto"}}>
          ...
          <div className="sf-in">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
              <div>
                ...
                <p style={{opacity:.5,fontSize:".9rem",marginTop:4}}>Subject → Unit → Topic</p>
              </div>
                ...
                <div key={sub.id} className="sf-glass" style={{padding:22}}>
                  ...
                  {sub.units.map(unit=>(
                    <div key={unit.id} style={{marginBottom:10}}>
                      <div style={{padding:"9px 12px",borderRadius:10,background:dark?"rgba(255,255,255,0.04)":"rgba(99,102,241,0.06)",
                      ...
                      {unit.topics.map(topic=>(
                        <div key={topic.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                          padding:"7px 12px 7px 22px",borderRadius:8,transition:"background .15s"}}
                          onMouseOver={e=>e.currentTarget.style.background=dark?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.03)"}
                          onMouseOut={e=>e.currentTarget.style.background="transparent"}>
                          <div onClick={()=>{setSelSub(sub);setSelUnit(unit);setSelTopic(topic);setTab("upload");}}
                            style={{cursor:"pointer",flex:1,fontSize:".84rem",opacity:.75}}>→ {topic.name}</div>
                      ...
                      {addMode===`tp-${unit.id}`?(
                        <div style={{display:"flex",gap:8,marginTop:6,paddingLeft:12}}>
                          <input className="sf-inp" placeholder="Topic name…" value={newName} onChange={e=>setNewName(e.target.value)}
                            style={{fontSize:".85rem",padding:"8px 12px"}} autoFocus onKeyDown={e=>e.key==="Enter"&&addTopic(sub,unit)}/>
                        ...
                        <button onClick={()=>{setAddMode(`tp-${unit.id}`);setNewName("");}} style={{marginTop:5,paddingLeft:22,fontSize:".8rem",
                          opacity:.38,background:"none",border:"none",cursor:"pointer",color:"inherit",transition:"opacity .2s"}}
                  ...
                  {addMode===`un-${sub.id}`?(
                    <div style={{display:"flex",gap:8,marginTop:12}}>
                      <input className="sf-inp" placeholder="Unit name…" value={newName} onChange={e=>setNewName(e.target.value)}
                        autoFocus onKeyDown={e=>e.key==="Enter"&&(setSelSub(sub),addUnit())}/>
                    ...
                    <button className="sf-btn sf-btn-g" onClick={()=>{setSelSub(sub);setAddMode(`un-${sub.id}`);setNewName("");}}
                      style={{width:"100%",justifyContent:"center",marginTop:14,fontSize:".85rem"}}>+ Add Unit</button>
                  )}
                </div>
              ))}
            </div>
        ...
        {tab==="upload" && (
          <div className="sf-in">
            ...
            <p style={{opacity:.5,fontSize:".9rem",marginBottom:28}}>Paste material and let AI build questions for you</p>
                  ...
                  <div key={sub.id} style={{marginBottom:12}}>
                    ...
                    {sub.units.map(unit=>(
                      <div key={unit.id} style={{paddingLeft:10}}>
                        ...
                        {unit.topics.map(topic=>(
                          <div key={topic.id} className={`sf-si ${selTopic?.id===topic.id?"sf-active":""}`}
                ...
                <div style={{display:"flex",flexDirection:"column",gap:16}}>
                  ...
                  <div className="sf-glass" style={{padding:"22px 24px"}}>
                    ...
                    <textarea className="sf-inp" value={matText} onChange={e=>setMatText(e.target.value)}
                      placeholder="Paste your notes, textbook text, or any study material here…" style={{minHeight:190}}/>
                  ...
                  <div className="sf-glass" style={{padding:"22px 24px"}}>
                    <h3 className="sf-syne" style={{fontWeight:700,marginBottom:6}}>⚡ Generate with AI</h3>
                    <p style={{fontSize:".86rem",opacity:.5,marginBottom:18}}>MCQ · True/False · Fill in Blank · Short Answer</p>
                    ...
                    {genMsg&&<div style={{marginTop:12,padding:"10px 14px",borderRadius:10,background:msgBg,color:msgColor,fontSize:".875rem"}}>{genMsg}</div>}
                  </div>
                    ...
                    <div className="sf-glass" style={{padding:"22px 24px"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:9,maxHeight:320,overflowY:"auto"}}>
                        {syncedTopic.questions.map((q,i)=>(
                          <div key={q.id} style={{padding:"11px 14px",borderRadius:12,background:dark?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.5)",
                            border:`1px solid ${dark?"rgba(255,255,255,0.06)":"rgba(99,102,241,0.1)"}`}}>
        ...
        <div className="sf-glass sf-up" style={{padding:30,maxWidth:700}}>
          ...
          {selTopic.questions.length===0?(
            <div style={{padding:"40px 20px",textAlign:"center",opacity:.4}}>
              ...
              <p style={{fontSize:".875rem",marginTop:8}}>Ask your teacher to generate questions for this topic</p>
            </div>
          ):(
            <>
              ...
              <div key={sub.id} className="sf-card">
                ...
                {sub.units.map(unit=>(
                  <div key={unit.id} style={{marginBottom:10}}>
                    ...
                    {unit.topics.map(topic=>(
                      <div key={topic.id} onClick={()=>{setSelSub(sub);setSelUnit(unit);setSelTopic(topic);}}
                        style={{padding:"9px 12px",borderRadius:10,cursor:"pointer",display:"flex",justifyContent:"space-between",
                          alignItems:"center",marginBottom:4,background:dark?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.5)",transition:"all .2s"}}
...
function Quiz({ quiz, dark, onFinish }) {
  ...
  // ✅ Fixed: timer is now adjustable (default 60s/question, capped 30–90)
  ...
  const finish = useCallback(()=>{ setDone(true); clearInterval(timerRef.current); },[]);

  const startQuiz = () => {
    setStarted(true);
    setTimeLeft(questions.length * secPerQ);
  };
  ...
  const next = () => { if(cur<questions.length-1){ setCur(c=>c+1); setText(""); } else finish(); };

  // Pre-quiz timer config screen
  if(!started) {
    return (
      <div style={{paddingTop:64,minHeight:"100vh",position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 24px"}}>
        <div className="sf-glass sf-up" style={{width:"100%",maxWidth:480,padding:36,textAlign:"center"}}>
          <div style={{fontSize:44,marginBottom:16}}>⏱</div>
          <h2 className="sf-syne" style={{fontSize:"1.6rem",fontWeight:800,marginBottom:6}}>{topic.name}</h2>
          <p style={{opacity:.5,marginBottom:28}}>{questions.length} questions · {subject.name}</p>

          <div className="sf-glass" style={{padding:"20px 24px",marginBottom:24,textAlign:"left"}}>
            <label className="sf-syne" style={{display:"block",fontSize:".75rem",fontWeight:700,opacity:.5,letterSpacing:".07em",textTransform:"uppercase",marginBottom:12}}>
              Time per question: <span style={{color:acc}}>{secPerQ}s</span>
            </label>
            <input type="range" min={30} max={90} step={15} value={secPerQ} onChange={e=>setSecPerQ(+e.target.value)} style={{width:"100%"}}/>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
              {[30,45,60,75,90].map(v=>(
                <span key={v} style={{fontSize:".75rem",opacity:v===secPerQ?1:.35,color:v===secPerQ?acc:"inherit",fontWeight:v===secPerQ?700:400}}>{v}s</span>
              ))}
            </div>
            <p style={{fontSize:".84rem",opacity:.45,marginTop:12}}>
              Total time: {Math.floor(questions.length*secPerQ/60)}m {(questions.length*secPerQ)%60}s
            </p>
          </div>

          <button className="sf-btn sf-btn-p" onClick={startQuiz} style={{width:"100%",justifyContent:"center",padding:"15px 0",fontSize:"1rem"}}>
            Start Quiz →
          </button>
        </div>
      </div>
  ...
  if(done) {
    ...
    questions.forEach(q=>{
      ...
      // ✅ Fixed: proper keyword-overlap scoring
      else if(q.type==="shortanswer") { correct += scoreShortAnswer(a||"", q.answer); }
    });
    correct = Math.round(correct);
    const pct = Math.round((correct/questions.length)*100);
    return <Results correct={correct} total={questions.length} pct={pct} questions={questions} answers={answers}
      topic={topic} subject={subject} dark={dark} onFinish={()=>onFinish({correct,total:questions.length,pct,topic:topic.name})}/>;
  }

  const mins = Math.floor((timeLeft||0)/60);
  const secs = (timeLeft||0)%60;
  const warn = timeLeft!==null && timeLeft<60;
    ...
    <div style={{paddingTop:64,minHeight:"100vh",position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"80px 24px 36px"}}>
      <div style={{width:"100%",maxWidth:680,marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <div className={warn?"sf-warn":""} style={{padding:"8px 16px",borderRadius:50,
            background:warn?"rgba(255,92,122,0.12)":dark?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.72)",
            border:`1px solid ${warn?"rgba(255,92,122,0.3)":dark?"rgba(255,255,255,0.08)":"rgba(99,102,241,0.15)"}`}}>
      ...
      <div className="sf-glass sf-up" style={{width:"100%",maxWidth:680,padding:32}} key={q.id}>
        ...
        {q.type==="truefalse"&&(
          <div style={{display:"flex",gap:12}}>
            {[true,false].map(val=>{
              ...
              return <button key={String(val)} className={cls} disabled={rev} onClick={()=>answer(val)}
                style={{flex:1,justifyContent:"center",padding:18,fontWeight:600,fontSize:"1rem"}}>{val?"✓ True":"✗ False"}</button>;
            })}
          </div>
        )}
        ...
        {q.type==="fillblank"&&(
          <div>
            <input className="sf-inp" placeholder="Type your answer…" value={text} onChange={e=>setText(e.target.value)} disabled={rev}
              onKeyDown={e=>{if(e.key==="Enter"&&text.trim()&&!rev){answer(text);setRevealed(p=>({...p,[q.id]:true}));}}}/>
            {rev&&(
              <div style={{marginTop:12,padding:"12px 14px",borderRadius:12,
                background:text.trim().toLowerCase()===q.answer.toLowerCase()?"rgba(79,255,176,0.1)":"rgba(255,92,122,0.1)"}}>
            ...
            {rev&&(
              <div>
                <div style={{marginTop:10,padding:"10px 14px",borderRadius:10,
                  background:scoreShortAnswer(text,q.answer)===1?"rgba(79,255,176,0.1)":scoreShortAnswer(text,q.answer)===0.5?"rgba(251,185,36,0.1)":"rgba(255,92,122,0.1)"}}>
                  <p style={{fontWeight:600,fontSize:".875rem",
                    color:scoreShortAnswer(text,q.answer)===1?"#4FFFB0":scoreShortAnswer(text,q.answer)===0.5?"#FBB924":"#FF5C7A"}}>
                    {scoreShortAnswer(text,q.answer)===1?"✓ Full credit":scoreShortAnswer(text,q.answer)===0.5?"~ Partial credit":"✗ Try reviewing the model answer"}
                  </p>
                </div>
                <div style={{marginTop:8,padding:"14px 16px",borderRadius:12,background:"rgba(251,185,36,0.08)",border:"1px solid rgba(251,185,36,0.2)"}}>
        ...
        <div className="sf-glass sf-up" style={{padding:"40px 36px",textAlign:"center",marginBottom:18}}>
          <svg width="124" height="124" style={{margin:"0 auto 20px",display:"block"}}>
            ...
            <circle cx="62" cy="62" r="46" fill="none" stroke={acc} strokeWidth="8"
              strokeDasharray={circ} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 62 62)"
        ...
        <div className="sf-glass" style={{padding:"22px 24px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:11}}>
            {questions.map((q,i)=>{
              const a = answers[q.id];
              let score = 0;
              if(q.type==="mcq")            score = a===q.answer?1:0;
              else if(q.type==="truefalse") score = a===q.answer?1:0;
              else if(q.type==="fillblank") score = (typeof a==="string"&&a.trim().toLowerCase()===q.answer.toLowerCase())?1:0;
              else if(q.type==="shortanswer") score = scoreShortAnswer(a||"",q.answer);
              const ok = score===1;
              const partial = score===0.5;

              return (
                <div key={q.id} style={{padding:"13px 15px",borderRadius:12,
                  background:dark?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.5)",
                  border:`1px solid ${ok?"rgba(79,255,176,0.2)":partial?"rgba(251,185,36,0.2)":a?"rgba(255,92,122,0.2)":dark?"rgba(255,255,255,0.06)":"rgba(99,102,241,0.1)"}`}}>
                  <div style={{display:"flex",gap:7,marginBottom:6,flexWrap:"wrap",alignItems:"center"}}>
                    <span style={{fontWeight:700,fontSize:".85rem",color:ok?"#4FFFB0":partial?"#FBB924":a?"#FF5C7A":"#FBB924"}}>
                      {ok?"✓":partial?"~":a?"✗":"—"}
                    </span>
                    <span className={`sf-badge sf-${q.type}`}>{q.type==="truefalse"?"T/F":q.type==="fillblank"?"Fill":q.type==="shortanswer"?"Short":"MCQ"}</span>
                    <span className={`sf-badge sf-${q.difficulty}`}>{q.difficulty}</span>
...
function App() {
  ...
  // Load subjects from Firestore once on mount
  useEffect(()=>{
    loadSubjectsFromFS().then(list=>{ setSubjects(list); setFsLoaded(true); });
  },[]);

  // Firebase auth listener — uses Custom Claims for role (not email comparison)
  useEffect(()=>{
    const unsub = auth.onAuthStateChanged(async fbUser=>{
      if(fbUser){
        const role   = await getUserRole(fbUser);
        const scores = await loadScoresFromFS(fbUser.uid);
        setUser({
          id:    fbUser.uid,
  ...
  const finishQuiz = async (result) => {
    ...
    await saveScoresToFS(user.id, newScores);
    setUser(p=>({...p, scores:newScores}));
    setQuiz(null);
  };

  // Loading screen — shown until both auth and Firestore are ready
  if(!authInit || !fsLoaded) {
    return (
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
        color:dark?"#fff":"#000",fontFamily:"'Syne',sans-serif",gap:14}}>
        <div style={{fontSize:42}}>📚</div>
        <div style={{display:"flex",alignItems:"center",gap:10,fontSize:"1.1rem",fontWeight:700}}>
          <div className="sf-spin" style={{width:22,height:22,borderWidth:3,borderTopColor:dark?"#4FFFB0":"#6366F1"}}/>
          Loading StudyFlow…
        </div>
        <p style={{opacity:.35,fontSize:".85rem"}}>Connecting to cloud…</p>
      </div>
    );
  }
